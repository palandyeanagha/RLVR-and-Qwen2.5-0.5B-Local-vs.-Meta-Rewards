...
def main():
    print("=" * 70)
    print("  VERL GRPO Training - GSM8K Full Training (Local Reward)")
    print("=" * 70)
    print()
    ...
    print("Configuration (local rewards):")
    print("  Dataset: GSM8K (full, preprocessed)")
    print("  Model: Qwen/Qwen2.5-0.5B")
    print("  Reward: Local (stepwise intermediate reasoning)")
    ...
    checkpoint_dir = f"{base_dir}/checkpoints_gsm8k_local_{timestamp}"
    log_dir = f"{base_dir}/logs_gsm8k_local_{timestamp}"
    
    # Main training command --- ONLY update these two lines for local reward!
    cmd = [
        ...
        # REWARD: update these lines!
        #f"custom_reward_function.path={base_dir}/local_rewards/local_rewards_CORRECTED.py",
        f"custom_reward_function.path={base_dir}/local_rewards/local_rewards_WORKAROUND.py",
        "custom_reward_function.name=local_reward_function",
        "critic.enable=false",
        # TRAINER OUTPUT
    ...
    print("  2. Analyze Pass@K metrics")
